(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ReduxSounds"] = factory();
	else
		root["ReduxSounds"] = factory();
})(this, function() {
...
module.exports = {
  isObjectWithValues: function isObjectWithValues(obj) {
...
var howlerIntegration = __webpack_require__(2);
  ...
  return function (store) {
    return function (next) {
      return function (action) {
        ...
        var keys = Object.keys(action.meta.sound);
        keys.forEach(function (key) {
          var val = action.meta.sound[key];
          if (Array.isArray(val)) {
            val.forEach(function (sound) {
              return howlerIntegration[key](sound.split('.'));
            });
          } else {
            howlerIntegration[key](val.split('.'));
          }
        });
        return next(action);
      };
...
var Howl = __webpack_require__(3).Howl;
...
function removeId(id) {
  Object.keys(playing).forEach(function (key) {
    return playing[key].delete(id);
  });
}

var validator = {
  get: function get(target, name, thisArg) {
    return name === 'initialize' ? function (soundsData) {
      return target.initialize.call(target, soundsData);
    } : function (soundName, spriteName) {
      ...
      var sound = target.sounds[soundName];
      ...
      var method = name === 'play' ? 'play' : 'howlMethod';
      return (_target$method = target[method]).call.apply(_target$method, [target, soundName, spriteName].concat(args));
    };
  }
};
...
module.exports = new Proxy({
  initialize: function initialize(soundsData) {
    ...
    this.sounds = soundNames.reduce(function (memo, name) {
      ...
      var sprites = soundOptions.sprites;
      if (isObjectWithValues(sprites)) {
        Object.keys(sprites).forEach(function (spriteName) {
          playing[name + spriteName] = new Set();
        });
      } else {
        playing[name] = new Set();
      }
  ...
  play: function play(soundName, spriteName) {
    ...
    var id = sound.play(spriteName);
    if (playing[soundName + spriteName]) {
      playing[soundName + spriteName].add(id);
    }
    return id;
  },
  howlMethod: function howlMethod(soundName, spriteName, method) {
    ...
    var sound = this.sounds[soundName];
    if (playing[soundName + spriteName]) {
      playing[soundName + spriteName].forEach(function (id) {
